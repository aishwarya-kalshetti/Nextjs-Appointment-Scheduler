...
export default function EditModal({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    await onSave({ ...appointment, name, date, time });
    setSaving(false);
  }
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <label className="flex flex-col">
            ...
            <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="mt-1 p-2 border rounded" />
          </label>

          <div className="flex justify-end gap-2">
            <button type="button" onClick={onClose} className="px-3 py-1 border rounded">Cancel</button>
            <button type="submit" disabled={saving} className="px-3 py-1 bg-slate-800 text-white rounded disabled:opacity-60">
              {saving ? "Saving..." : "Save"}
            </button>
          </div>
        </form>
